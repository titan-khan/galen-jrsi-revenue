...
export const derivedPkbMetrics: MetricDefinition[] = [
  ...
  // ─── P1.7 Phone Coverage × Segment ──────────────────────────────────────
  {
    ...BASE,
    id: "M-DERIV-003",
    name: "Cakupan HP Kelompok Terendah",
    description:
      `Cakupan nomor HP di kelompok kepatuhan paling rendah — yaitu "${N.S2}" Kendaraan Hantu dengan 4%. Angka ini penting karena menentukan apakah saluran digital cukup, atau wajib pakai surat / kunjungan tim lapangan SAMSAT. Detail cakupan kelompok lain ada di insight.`,
    ...
    displayData: {
      ...
      subtitle: `Cakupan HP di "${N.S2}" — kelompok paling sulit dijangkau`,
      ...
      insight: {
        text: `"${N.S2}" hanya ${PHONE_BY_SEG.S2}% punya HP, "${N.S1}" ${PHONE_BY_SEG.S1}% — wajib didatangi langsung (surat, RT-RW, tim SAMSAT). Bandingkan dengan "${N.H1}" ${PHONE_BY_SEG.H1}% & "${N.K1}" ${PHONE_BY_SEG.K1}% yang siap kampanye WhatsApp.`,
  ...
  // ─── P1.8 Pyramid Deviation vs Framework Piramida Kepatuhan Pajak ──────────────────────────
  {
    ...
    displayData: {
      ...
      currentValue: `${pyramidDeviation.H1.toFixed(2)} poin`,
      changePercent: pyramidDeviation.H1,
      changeAbsolute: `${pyramidDeviation.H1.toFixed(2)} persen poin di bawah target`,
      status: "critical",
      sparklineData: SPARK_PYRAMID_DEV,
      insight: {
        text: `"${N.H1}" lebih rendah ${Math.abs(pyramidDeviation.H1).toFixed(2)} persen poin dari target 40% — sinyal kepatuhan struktural lebih lemah dari ekspektasi. "${N.S2}" lebih banyak ${pyramidDeviation.S2.toFixed(2)} poin dari target (beban kendaraan tidak teridentifikasi). Butuh dua aksi paralel: program retensi pembayar patuh + pembersihan registrasi.`,
        boldParts: [`${Math.abs(pyramidDeviation.H1).toFixed(2)} persen poin`, N.H1, N.S2],
      },
    },
  },
  ...
  // ─── P1.9 Vehicle Age × Segment ─────────────────────────────────────────
  {
    ...
    displayData: {
      ...
      insight: {
        text: `"${N.S2}" rata-rata ${AGE_BY_SEG.S2} tahun + hanya 4% punya HP → kandidat penghapusan registrasi, bukan target penagihan. Pembanding: "${N.M2}" rata-rata ${AGE_BY_SEG.M2} tahun (denda akumulasi sering melebihi nilai jual), "${N.K1}" ${AGE_BY_SEG.K1} tahun.`,
  ...
  return order.map((code) => {
    ...
    return {
      ...
      displayData: {
        ...
        changeAbsolute: `${devSign}${dev.toFixed(2)} persen poin dari target`,
        status,
        sparklineData: sparkline,
        insight: {
          text: `${seg.n.toLocaleString("id-ID")} kendaraan (${seg.pct.toFixed(2)}% dari total) — ${devSign}${dev.toFixed(2)} persen poin ${dev < 0 ? "di bawah" : "di atas"} target panduan ${FW[code as keyof typeof FW]}%. ${phone}% punya nomor HP. ${TREATMENT[code]}`,
          boldParts: [
            `${seg.n.toLocaleString("id-ID")} kendaraan`,
            `${seg.pct.toFixed(2)}%`,
            `${devSign}${dev.toFixed(2)} persen poin`,
            `${phone}%`,
          ],
        },
      },
